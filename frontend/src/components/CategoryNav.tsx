        ...
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide px-1">
          ...
          {CATEGORIES.map((cat) => (
            <button
              key={cat.id}
              onClick={() => {
                const isExpanding = cat.id !== activeParent;
                setActiveParent(isExpanding ? cat.id : null);

                // USER FIX: Always select the category when clicking, even if it has children
                // This allows filtering by "Digital" (Parent) immediately
                if (isExpanding) {
                  onSelect(cat);
                } else {
                  onSelect(null); // Deselect if collapsing/toggling off
                }
              }}
