        ...
        <div className="relative aspect-[3/2] overflow-hidden bg-gray-100">
          ...
          {isTicket && ticketInfo && (
            <div className="absolute top-3 left-3 flex flex-col items-start gap-1">
              ...
              {/* Premium Price Indicator (DB Algo Demo) */}
              {product.premiumPercent !== undefined && (
                <span className={`px-2 py-0.5 text-[10px] font-mono font-bold rounded-full border shadow-sm backdrop-blur-md mt-1
                    ${product.premiumPercent > 10
                    ? 'bg-red-500/90 text-white border-red-400'
                    : 'bg-emerald-500/90 text-white border-emerald-400'}`}>
                  {product.premiumPercent > 0 ? `+${product.premiumPercent}%` : '定可'}
                </span>
              )}
            </div>
          )}
          ...
          {!isTicket && deviceDetails && (
            <div className="absolute bottom-3 left-3 flex flex-col gap-1">
              {/* SKU Badge (DB Standard Product Demo) */}
              {product.productCode && (
                <span className="px-1.5 py-0.5 text-[9px] font-mono text-xs text-indigo-800 bg-indigo-50 border border-indigo-200 rounded self-start">
                  SKU: {product.productCode}
                </span>
              )}

              <span className="px-2 py-1 text-[10px] font-medium bg-white/90 backdrop-blur-sm text-gray-700 rounded-md shadow-sm self-start">
